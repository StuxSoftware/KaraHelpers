...
function beziere (pct, p, as_string) if _G.type(as_string) == "nil" then as_string=true end; local function fac(n) local k = 1; if n > 1 then for i = 2,n do k = k * i; end; end; return k; end; local function bin(i, n) return fac(n) / (fac(i)*fac(n-i)) end; local function bernstein(pct, i, n) return bin(i, n) * pct^i * (1-pct)^(n-i); end; local point = {0,0}; local n = #p - 1; for i=0, n do local bern = bernstein(pct, i, n); point[1] = point[1] + p[i+1][1] * bern; point[2] = point[2] + p[i+1][2] * bern; end; if as_string then resultstr = "\\pos("; resultstr = resultstr .. point[1] .. "," .. point[2]; return resultstr .. ")"; else return point end; end

-- Set the maximal loop execution proportional to the number of frames shown.
function frames(length, retime) if tenv._G.type(length) == "nil" then length = (1000/(_G.frame_from_ms(1000) or 20)) end; if _G.type(retime) == "nil" then retime=true; end; local start, stop = tenv.line.start_time, tenv.line.end_time; if retime then tenv.retime("set", start + (tenv.j) * length, start + tenv.j*length); end; tenv.maxloop(math.max(math.floor((stop-start) / length), 1)); return ""; end;
